...
const CreatePost = () => {
    ...
    const handleVehicleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        setForm((prev) => ({
            ...
            vehicle: {
                ...
                ...(name === "model" && { year: "" }),
                ...(name === "vehicleType" && { motorcycleInfo: { hasSidecar: false }})
            },
        }));
    };
    ...
    const handleSubmit = async () => {
        ...
        try {
            const res = await axiosPrivate.post("api/posts", form);
        ...
        <Paper sx={{ mt: 10, p: 4, mx: 40, backgroundColor: '#f1f1f1' }}>
                    ...
                    <Box display="flex" flexDirection="column" gap={1}>
                        {form.images.map((img, idx) => (
                            <Box key={idx} display="flex" gap={1}>
                                <Typography variant="body2" sx={{flex: 1}}>{img}</Typography>
            ...
            <Box mt={4}>
                ...
                <TextField
                    ...
                    sx={commonFieldSx}
                >
                    {brands.map(br => <MenuItem key={br} value={br}>{br}</MenuItem>)}
                </TextField>
